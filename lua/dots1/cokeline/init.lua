...
local hlgroups_utils = require("cokeline.hlgroups")

local is_picking_focus = require("cokeline.mappings").is_picking_focus
local is_picking_close = require("cokeline.mappings").is_picking_close
local get_hex = hlgroups_utils.get_hl_attr
local augroup = vim.api.nvim_create_augroup
local aucmd = vim.api.nvim_create_autocmd
local terminal_id = augroup("Change Color Scheme", {clear = false})

local function create_config()
  hlgroups_utils._cache_clear()
  local errors_fg = get_hex("DiagnosticError", "fg")
  local warnings_fg = get_hex("DiagnosticWarn", "fg")
  local info_fg = get_hex("DiagnosticInfo", "fg")
  local hint_fg = get_hex("DiagnosticHint", "fg")
  local status_line_bg = get_hex("StatusLine", "bg")
  local status_line_fg = get_hex("StatusLine", "fg")
  local green = vim.g.terminal_color_2
  local red = vim.g.terminal_color_1
  local yellow = vim.g.terminal_color_3

  vim.api.nvim_set_hl(0, "TabLine", {bg = status_line_bg, fg = status_line_fg})
  cokeline.setup(
    {
      default_hl = {
        fg = function(buffer)
...
keymap("n", "<leader>bda", ":%bdelete<CR>", {silent = true})
keymap("n", "gb", "<Plug>(cokeline-pick-focus)", {silent = true})
keymap("n", "<leader>bc", "<Plug>(cokeline-pick-close)", {silent = true})
keymap("n", "H", "<Plug>(cokeline-focus-prev)", {silent = true})
keymap("n", "L", "<Plug>(cokeline-focus-next)", {silent = true})
